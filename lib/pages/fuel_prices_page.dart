...
class _FuelPricesPageState extends State<FuelPricesPage> with SingleTickerProviderStateMixin {
  ...
  void _showEditStationDialog(Map<String, dynamic> station, Map<String, dynamic>? overrideData) {
    showDialog(
      context: context,
      builder: (context) => _EditFuelPriceDialog(
        dataService: _dataService,
  ...
  Future<void> _deleteOverride(int id) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        title: const Text('Delete Override', style: TextStyle(color: Colors.white)),
        content: const Text('Are you sure you want to remove this station override?', style: TextStyle(color: Colors.white70)),
        actions: [
                ...
                child: ElevatedButton.icon(
                  onPressed: () {
                    if (_tabController.index == 0) {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text('Click the Edit button next to a Station to override.')),
                      );
                    }
                  },
  ...
  Widget _buildRegionalTab() {
    final regionalPrices = _fuelPrices.where((p) => p['station_id'] == null).toList();
        ...
        child: SingleChildScrollView(
          child: DataTable(
            ...
            rows: regionalPrices.map((region) {
              return DataRow(
                cells: [
                  DataCell(Text(region['region'] ?? 'Unknown', style: const TextStyle(fontWeight: FontWeight.bold))),
  ...
  Widget _buildStationTab() {
    final overridePrices = _fuelPrices.where((p) => p['station_id'] != null).toList();
        ...
        child: SingleChildScrollView(
          child: DataTable(
            ...
            rows: _stations.map((station) {
              ...
              final overrideData = overridePrices.cast<Map<String, dynamic>?>().firstWhere(
              ...
              if (displayData == null) {
                ...
                displayData = _fuelPrices.cast<Map<String, dynamic>?>().firstWhere(
                  (p) => p != null && p['station_id'] == null && p['region'] == stationRegion,
                  orElse: () => null,
                );
              }
                ...
                cells: [
                  DataCell(
                    Column(
                      ...
                      children: [
                        Text(station['name'] ?? 'Unknown', style: const TextStyle(fontWeight: FontWeight.bold)),
                        Text('ID: ${station['station_id'] ?? ''}', style: const TextStyle(fontSize: 10, color: Colors.white54)),
                      ],
                    ),
                  ),
                  ...
                  DataCell(Text('${displayData?['premium_diesel'] ?? '-'} MMK')),
                  DataCell(
                    Container(
                      padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                      decoration: BoxDecoration(
                        color: hasOverride ? Colors.orange.withOpacity(0.2) : Colors.green.withOpacity(0.2),
                        borderRadius: BorderRadius.circular(8),
                      ),
                      child: Text(
                        hasOverride ? 'Overridden' : 'Regional Default', 
                        style: TextStyle(
                          color: hasOverride ? Colors.orangeAccent : Colors.greenAccent,
                          fontSize: 12,
                        ),
                      ),
                    ),
                    ...
                    Row(
                      ...
                      children: [
                        IconButton(
                          ...
                          onPressed: () => _showEditStationDialog(station as Map<String, dynamic>, overrideData),
                        ),
                        if (hasOverride)
                          IconButton(
                            icon: const Icon(Icons.delete, color: Colors.redAccent, size: 20),
                            tooltip: 'Remove Override',
                            onPressed: () => _deleteOverride(overrideData['id']),
                          ),
                      ],
                    ),
                  ),
...
class _EditFuelPriceDialogState extends State<_EditFuelPriceDialog> {
  ...
  void initState() {
    ...
    _octane92Controller = TextEditingController(text: data['octane_92']?.toString() ?? '');
    _octane95Controller = TextEditingController(text: data['octane_95']?.toString() ?? '');
    _dieselController = TextEditingController(text: data['diesel']?.toString() ?? '');
    _premiumDieselController = TextEditingController(text: data['premium_diesel']?.toString() ?? '');
    _fetchRegions();
  }
  ...
  Widget build(BuildContext context) {
    String title = widget.isRegional 
        ? (widget.priceData == null ? 'Add Regional Prices' : 'Edit Regional Prices')
            ...
            children: [
              if (widget.isRegional) ...[
                DropdownButtonFormField<String>(
                  ...
                  onChanged: widget.priceData != null ? null : (v) => setState(() => _selectedRegion = v),
                  validator: (v) => v == null ? 'Required' : null,
                ),
                const SizedBox(height: 16),
              ],

              _buildField(_octane92Controller, "Octane 92 (MMK)", isNumber: true),
              const SizedBox(height: 16),
              _buildField(_octane95Controller, "Octane 95 (MMK)", isNumber: true),
              const SizedBox(height: 16),
              _buildField(_dieselController, "Diesel (MMK)", isNumber: true),
              const SizedBox(height: 16),
              _buildField(_premiumDieselController, "Premium Diesel (MMK)", isNumber: true),
            ],
          ),
        ),
      ),
      actions: [
        ...
        ElevatedButton(
          ...
          child: _isSaving 
            ...
            : const Text('Save Prices', style: TextStyle(color: Colors.white)),
        ),
      ],
    );
  }
  ...
  Widget _buildField(TextEditingController controller, String label, {bool isNumber = false}) {
    return TextFormField(
      ...
      validator: (v) {
        ...
        if (isNumber && int.tryParse(v) == null) return 'Must be a valid integer';
        return null;
      },
    );
  }
